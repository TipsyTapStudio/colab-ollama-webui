...
| # | リスク | 対処 | 状態 |
|---|---|---|---|
| 1 | Open WebUI の Python バージョン制約。公式ドキュメント上は 3.11 / 3.12 対応だが、3.11 のみとする情報もあり錯綜している。Colab の標準は 3.12 系 | 3.11 の venv を別途作り、そこに導入する | 未検証 |
| 2 | SQLite × Drive(FUSE) のファイルロック問題 | 4章の同期方式で回避 | 未検証 |
| 3 | Quick Tunnel の取得失敗。Cloudflare 自身がアカウントなしトンネルの稼働保証はないと明言 | リトライを入れる。恒常的に失敗するなら ngrok を検討（初版ではリトライのみ） | 未検証 |
| 4 | WebSocket が通らない可能性 | cloudflared 経由なら通る見込み。ダメなら `ENABLE_WEBSOCKET_SUPPORT=false` でポーリングに落とす | 未検証 |
