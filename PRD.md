...
**決定（2026-08-10）: 毎回 pull する。**

実測比較の前にユーザー判断で確定した。理由:

- モデルは1個 5〜16GB あり、Drive に置くと無料枠（15GB）を圧迫する
- Colab のダウンロード回線は速く、毎回の pull の待ち時間は許容範囲だった
- Drive 配置には I/O が遅く初回ロードで不利になりうる懸念もあった

これに伴いフェーズ4の「置き場の実測比較」は不要になった。今後起動時間の短縮を追う場合は、
venv 構築（pip install の約5分）のキャッシュ化が残る候補。
...
| 2 | SQLite × Drive(FUSE) のファイルロック問題 | 4章の同期方式で回避 | 解消（2026-08-10 ランタイム削除→復元で履歴の残存を実機確認 = AC3/AC4 達成） |
...
| 5 | トンネルURLに認証がない。Open WebUI は初回アクセス者が管理者になる | 起動直後に自分でアカウントを作り、サインアップ無効を確認する。**手順に組み込む** | 解消（v0.11 は New Sign Ups が初期値オフ＋新規登録者は保留承認制であることを実機確認） |
